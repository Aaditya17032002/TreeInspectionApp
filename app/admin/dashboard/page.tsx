...
export default function AdminDashboard() {
  ...
  const loadInspections = async () => {
    try {
      ...
      // Filter to show only completed inspections
      setInspections(data.filter(inspection => inspection.status === 'Completed'))
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b p-4 bg-white sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <h1 className="text-2xl font-bold">Admin Dashboard - Completed Inspections</h1>
          <Button
            variant="ghost"
            onClick={() => {
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        {inspections.length === 0 ? (
          <p className="text-center text-gray-500">No completed inspections found.</p>
        ) : (
                ...
                <CardContent>
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button className="w-full mt-4">Review Inspection</Button>
                    </DialogTrigger>
